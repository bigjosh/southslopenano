...
- Both `index.html` (home) and `mosaic.html` (+ 6 variants) rewritten on Bootstrap 5.3.8 (CDN) per Josh's request after iterative custom CSS bumps weren't readable on his Android Chrome. Uses Bootstrap's typography hierarchy (`display-4`/`display-3` hero, `.lead`, `.h3` for sections, `.btn-lg`), `data-bs-theme="dark"` for dark mode, with custom overrides for the cyan accent (`.btn-cyan`, `.text-accent`, `.hr-cyan`, `.deadline` chip, link color). `html { font-size: 18px }` to bump every rem-based size ~12.5% over Bootstrap's defaults. Container max-width 720px to keep readable line lengths.
