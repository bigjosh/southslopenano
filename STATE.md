...
- Variant attribution strategy: per-variant inbound email addresses (e.g. `tile@`, `pixel@`, `claim@`...) under a catch-all on `southslopenano.com` instead of UTM-encoded subjects. Picked over UTM-via-subject because: words read better in Josh's inbox than codes, and a catch-all means no extra infrastructure beyond what Josh is setting up anyway. Specific address-to-variant mapping pending Josh's choice.
